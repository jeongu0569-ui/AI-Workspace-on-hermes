...
Codmes Search의 전체 흐름과 Ollama/LM Studio VLM 검증 기록은 [docs/codmes-search-explained.md](docs/codmes-search-explained.md)에 쉽게 정리되어 있습니다.
PDF 필기와 텍스트/이미지 annotation을 Windows/Android까지 가져가기 위한
공통 저장 계약은 [docs/platform-neutral-pdf-annotations.md](docs/platform-neutral-pdf-annotations.md)에 정리되어 있습니다.
...
필기 저장 포맷은 플랫폼 중립을 목표로 합니다. iOS/iPadOS에서는 PencilKit을
입력/렌더링 어댑터로 사용하지만, 저장 시에는 기존 `inkDataBase64` 호환
필드와 함께 정규화된 `inkStrokes` JSON도 기록합니다. `inkStrokes`는
0~1 좌표, 압력, 시간, 색상, 두께를 담는 Codmes 공통 stroke 포맷이므로
macOS, Windows, Android/Galaxy Tab 클라이언트가 PencilKit 없이도 같은
필기 데이터를 렌더링할 수 있습니다. macOS PDF 화면은 첫 단계로
`inkStrokes`를 PDFKit ink annotation으로 표시하고, 펜/지우개 모드에서
